...
프로그래밍에서 **조건문**은 상황에 따라 다른 행동을 하고 싶을 때 사용합니다.
마치 "비가 오면 우산을 쓰고, 아니면 그냥 간다"와 같습니다.
...
**2. 논리 연산자 (조건 합치기)**
* 'A && B' (AND): A와 B가 **모두** 참이어야 합니다. (예: 배도 고프고 && 돈도 있다면)
* '!A' (NOT): A가 **아니라면** 참입니다. (예: !비가온다 -> 비가 안 오면)

**3. 예시 코드 (놀이공원 입장)**
// 키가 140 이상이고 티켓이 있어야 입장 가능
if (height >= 140 && hasTicket) {
    System.out.println("입장하세요!");
} else {
    System.out.println("못 들어가요.");
}

---

**저스틴:** "심플해! 참가비는 500벨 이상이어야 하고(AND), 그리고 주머니는 꽉 차있으면 안돼(NOT). 이 두 가지가 동시에 충족되어야 '참가(True)'야!"`,
            prompt: '참가 자격(canJoin)을 결정하는 if문을 작성하세요. (money >= 500 && !isPocketFull)',
...
컴퓨터에게 똑같은 일을 100번 시키려면 어떻게 할까요? 코드를 100줄 쓰는 대신 **반복문**을 사용합니다.

**1. For 문 (횟수가 정해져 있을 때)**
"10번만 해!" 처럼 횟수가 명확할 때 씁니다.
// 0부터 4까지 총 5번 반복
for (int i = 0; i < 5; i++) {
    System.out.println("점프!");
}
* 'int i = 0': 시작점 (0부터 시작)
* 'i < 5': 끝나는 조건 (5보다 작을 때까지)
* 'i++': 변화 (하나씩 증가)

**2. While 문 (조건이 만족될 때까지)**
"배부를 때까지 먹어!" 처럼 횟수보다는 **목표 달성**이 중요할 때 씁니다.
int dumplings = 0;
while (dumplings < 10) { // 만두를 10개 먹을 때까지
    eatDumpling();
    dumplings++; // 먹은 개수 증가
}
...
**(2) While문 모드:** "10마리 잡을 때까지 집에 안 가!"`,
            prompt: 'For 루프(횟수 반복)와 While 루프(조건 반복)를 모두 구현하여 로봇을 완성하세요.',
            template: `public class FishingBot {
    public void startFishingBot() {
        // (1) For문 모드: "딱 10번만 던져!"
        System.out.println("=== For문 로봇 가동 ===");
        
        // [작성할 코드]: for문을 사용하여 10번 반복하세요.
        // 힌트: for (int i = 0; i < 10; i++) { ... }
        
           fishing(); // 이 함수를 호출해야 낚시를 합니다.
        ...
        // [작성할 코드]: while문을 사용하여 fishCount가 10 미만일 동안 반복하세요.
        // 힌트: while (fishCount < 10) { ... }
        // 조건: fishing() 함수가 true를 반환하면 fishCount를 1 증가시키세요 (fishCount++).
